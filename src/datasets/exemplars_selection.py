...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader, transform):
        clock0 = time.time()
        # first step: determine how many exemplars per class (or in total)
        exemplars_per_class = self._exemplars_per_class_num(model)
        # second step: select the batch size for the selection loader, which should be the same as the training loader, otherwise the order of the samples will be different and the selected indices will not correspond to the correct samples in the original dataset.
        if trn_loader.batch_size is None:
            batch_size = trn_loader.batch_sampler.true_batch_size
        else:
            batch_size = trn_loader.batch_size
        # third step: select the indices of the samples to be stored as exemplars, using the selection strategy defined in the subclass. This is done by creating a new loader with the same dataset but with the selection transform, and then applying the selection strategy on this new loader. The order of the samples in this new loader should be the same as the original loader, otherwise the selected indices will not correspond to the correct samples in the original dataset.
    ...
    def _exemplars_per_class_num(self, model: LLL_Net):
        ...
        # use the task_cls attribute of the model to determine the number of classes seen so far, which is the sum of the task_cls list.
