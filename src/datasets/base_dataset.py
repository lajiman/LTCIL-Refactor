...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x = Image.open(self.images[index]).convert('RGB')   # read the image from the path and convert it to RGB format, rather than reading the array from memory.
        x = self.transform(x)
        y = self.labels[index]
        return x, y
...
def get_data(path, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None,lt = False,ltio =False ):
    ...
    # memory_dataset use trn_data and tst_data directly from memory, while base_dataset read the filenames and labels from txt files. The format of the txt files is: image_path label, for example: /path/to/image1.jpg 0, /path/to/image2.jpg 1, ...
    ...
    for this_image, this_label in trn_lines:
        # if the image path is not absolute, then join it with the root path
    ...
    # here is the main difference between memory_dataset and base_dataset
    ...
    for this_image, this_label in trn_lines:
        if not os.path.isabs(this_image):
            this_image = os.path.join(path, this_image) # here just store the path, so no need for "data[tt][split]['x'] = np.asarray(data[tt][split]['x'])"
        this_label = int(this_label)
        if this_label not in class_order:
            continue
...
# this function should be carefully designed in acvil case. It should be one of the mean difference
