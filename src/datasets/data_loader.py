...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1):
    ...
    taskcla = []    # example, taskcla = [(0, 10), (1, 10), (2, 10)], which means task 0 has 10 classes, task 1 has 10 classes, task 2 has 10 classes.
    dataset_offset = 0
    for idx_dataset, cur_dataset in enumerate(datasets, 0): # datasets can be a list of datasets, for example, datasets = ['mnist', 'cifar100'], which means we want to use both MNIST and CIFAR100 in the continual learning scenario. In that case, we will first load MNIST and then load CIFAR100, and we will apply an offset to the class labels of CIFAR100 to make sure they are different from the class labels of MNIST. For example, if MNIST has 10 classes (0-9), then we will apply an offset of 10 to the class labels of CIFAR100, so that they will be in the range of 10-109.
        # get configuration for current dataset
        dc = dataset_config[cur_dataset]

        # I'm considering to add a dc['specific'] field to the dataset_config, to adapt to more special datasets with specific needs. For example, random seeds
        # transformations
        ...
        curtaskcla = [(tc[0] + idx_dataset * num_tasks, tc[1]) for tc in curtaskcla]    # example, if curtaskcla = [(0, 10), (1, 10), (2, 10)], and idx_dataset = 1, num_tasks = 3, then curtaskcla will be [(3, 10), (4, 10), (5, 10)], which means task 3 has 10 classes, task 4 has 10 classes, task 5 has 10 classes. This is to make sure that the task ids are different for different datasets, and they are in the range of [0, num_tasks * num_datasets - 1].
