...
class ExemplarsDataset(MemoryDataset):
    ...
    def __init__(self, transform, class_indices,
        ...
        # two mode, either a fixed total number of exemplars, or a fixed number of exemplars per class. Can only choose one.
        ...
        # dynamically register the exmaplar strategy.
    ...
    def extra_parser(args):
        ...
        # _group = parser.add_mutually_exclusive_group(required=True)    # at least one of the two limits should be specified, otherwise the exemplar management is not active.
