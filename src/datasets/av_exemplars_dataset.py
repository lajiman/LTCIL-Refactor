...
class AVExemplarsDataset(ExemplarsDataset):
    """
    Exemplar storage for AV features.
    Inherits ExemplarsDataset to satisfy main_incremental type check:
        issubclass(Appr_ExemplarsDataset, ExemplarsDataset)
    ...
    def __init__(self, transform, class_indices,
                 num_exemplars=0, num_exemplars_per_class=0, exemplar_selection='random'):
        # do NOT call super().__init__ from MemoryDataset path assumptions directly
        # but we still reuse ExemplarsDataset-compatible fields and selector creation
        self.images = []
        self.labels = []
        self.transform = transform
        self.class_indices = class_indices
        ...
        self.max_num_exemplars_per_class = num_exemplars_per_class
        self.max_num_exemplars = num_exemplars
        assert (num_exemplars_per_class == 0) or (num_exemplars == 0), \
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform):
        if self._is_active():
            ...
            self.labels = [int(y) for y in self.labels]
